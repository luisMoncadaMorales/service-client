...
public class ClientServiceTest {
    ...
    public void setup(){
        ...
        Mockito.when(repository.saveClient(clientDTOConvert)).thenReturn(clientDTO);
        ...
        Mockito.when(photoFeign.photoById(1052,"cc")).thenReturn(ResponseEntity.ok(photoDTO));
        Mockito.when(photoConvertService.photoToClient(clientDTO,photoDTO)).thenReturn(clientDTOConvert);
        ...
        Mockito.when(photoConvertService.clientToPhoto(clientDTOConvert)).thenReturn(photoDTO);
        Mockito.when(photoFeign.deleteById(1052,"cc")).thenReturn(ResponseEntity.ok("removed"));
        Mockito.when(photoFeign.deleteById(1053,"cc")).thenReturn(ResponseEntity.ok("not found"));
    }
    @Test
    ...
    public void saveClientTest() {
        ClientDTO clientDTOResult= service.saveClient(this.clientDTOConvert);
        Assertions.assertThat(clientDTOResult).isEqualTo(this.clientDTOConvert);
    }
    @Test
